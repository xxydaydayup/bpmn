...
export const { readApproval, approvalPatch } = await load('approval')
...
export const descriptor = JSON.parse(readFileSync(new URL('../src/bpmn/workflow-moddle.json', import.meta.url), 'utf8'))
export const graph = definitions => definitions.rootElements.filter(item => item.$type === 'bpmn:Process').map(process => ({
  ...
  nodes: (process.flowElements ?? []).filter(item => item.$instanceOf('bpmn:FlowNode')).map(item => ({
    ...
    approval: item.$type === 'bpmn:UserTask' ? readApproval(item) : undefined,
